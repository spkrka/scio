...
/**
 * Test utilities for SMB operations.
 * Must be in same package as BucketMetadata to access package-private methods.
 */
public class SmbTestUtils {
   ...
   *
   * Uses GenericDatumReader to read the file, which works for both GenericRecord
   * and specific record types (specific records are GenericRecords internally).
   *
  ...
  public static <T> List<T> readAvroFile(BucketMetadata<?, ?, T> metadata, File file) throws IOException {
