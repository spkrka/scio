 ...
 *
 * Manages temp directories, writes test data as SMB files, and provides
 * convenient accessors for reading in tests.
 *
 * Example usage:
 * {{{
...
class SMBTestContext private (
   ...
   *
   * @param name Logical name for this input (used in avroRead)
   * @param keyClass Key class for bucketing
   * @param keyField Name of the key field in the record
   * @param data Test data to write
   * @tparam K Key type
   ...
   *
   * @param name Logical name for this input
   * @param keyClass Primary key class
   * @param keyField Primary key field name
   * @param keyClassSecondary Secondary key class
   * @param keyFieldSecondary Secondary key field name
   * @param data Test data to write
   * @tparam K1 Primary key type
   * @tparam K2 Secondary key type
   * @tparam T Record type
   */
   ...
   *
   * @param name Logical name of the input (registered with avroInput)
   * @tparam T Record type
   */
  def avroRead[T <: org.apache.avro.specific.SpecificRecord](name: String)(implicit ct: scala.reflect.ClassTag[T]): SortedBucketIO.Read[T] = {
    val input = inputs.getOrElse(name, throw new IllegalArgumentException(s"No input registered with name: $name"))
   ...
   *
   * @param name Logical name of the input
   */
  def pathFor(name: String): String = {
    inputs.getOrElse(name, throw new IllegalArgumentException(s"No input registered with name: $name")).getPath
  }

  /**
   * Run the test with this context.
   *
   * Writes all registered inputs to temp directories, runs the test block,
   * then cleans up temp directories.
   *
   * @param testBlock Test code to execute
   * @tparam T Return type of test block
   */
...
object SMBTestContext {
   ...
   *
   * @param numBuckets Number of buckets for SMB outputs (default: 2)
   * @param numShards Number of shards per bucket (default: 1)
   */
 ...
 */
private sealed trait SMBTestInput[T] {
  def getPath: String
...
private class AvroTestInput[K, T <: org.apache.avro.specific.SpecificRecord](
  ...
  numBuckets: Int,
  numShards: Int
)(implicit coder: Coder[T]) extends SMBTestInput[T] {
...
private class AvroTestInputWithSecondaryKey[K1, K2, T <: org.apache.avro.specific.SpecificRecord](
  ...
  override def writeData(ctx: SMBTestContext): Unit = {
    ...
    sc.parallelize(data)
      .saveAsSortedBucket(
        AvroSortedBucketIO
          .write(keyClass, keyField, keyClassSecondary, keyFieldSecondary, data.head.getClass.asInstanceOf[Class[T]])
          .to(path)
          .withNumBuckets(numBuckets)
          .withNumShards(numShards)
      )
