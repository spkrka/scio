 ...
 *
 * The bug: When an AvroCoder is created with a custom DatumFactory (like SpecificRecordDatumFactory),
 * serializing and deserializing the coder does not preserve the DatumFactory. This causes
 * deserialization to fail with InstantiationException.
 *
 * Context: Beam serializes coders when sending them across worker boundaries or storing them
 * in the pipeline graph. If the DatumFactory is not properly preserved, the deserialized coder
 * will use the default SpecificDatumReader which may fail to instantiate records.
 */
class BeamAvroCoderSerializationTest extends AnyFlatSpec with Matchers {
  ...
  "AvroCoder with SpecificRecordDatumFactory" should "preserve DatumFactory after serialization" in {
    ...
    try {
      ...
      case e: CoderException =>
        ...
        throw e
      case e: Exception =>
        println(s"[DEBUG] Exception when decoding with fresh coder: ${e.getClass.getName}: ${e.getMessage}")
        e.printStackTrace()
        throw e
    }
  }
  ...
  it should "work when using default Avro coder without custom DatumFactory" in {
    ...
    println(s"[DEBUG Control] Successfully decoded with fresh default coder: ${decodedUser.getFirstName}")
