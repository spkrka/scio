...
object SmbTestHelper extends Matchers {
   ...
   *
   * Note: This does NOT verify record-level bucket assignment or sort order
   * due to complexity of handling Avro GenericRecord vs. specific record types.
   * Use readAllSmbRecords() separately to verify actual data if needed.
   *
   * @param outputDir Path to SMB output directory
   * @param expectedNumBuckets Expected number of buckets
   * @param keyExtractor Function to extract key from record (unused, kept for API compatibility)
   * @param expectedHashType Optional expected hash type (if None, any hash type is accepted)
   * @tparam T Record type
   * @return Assertion indicating success or failure
   */
  ...
  /**
   * Read all records from an SMB output directory.
   * Useful for verifying data correctness in tests.
   *
   * @param outputDir Path to SMB output directory
   * @tparam T Record type
   * @return List of all records from all buckets
   */
  ...
  /**
   * Count unique keys in SMB output.
   * Useful for verifying shared computation in multi-output scenarios.
   *
   * @param outputDir Path to SMB output directory
   * @param keyExtractor Function to extract key from record
   * @tparam T Record type
   ...
   *
   * @param file Avro file to read
   * @param metadata metadata to extract schema
   * @tparam T record type
