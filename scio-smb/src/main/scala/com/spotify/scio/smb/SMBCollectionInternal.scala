...
private[smb] class SMBPCollectionConsumer[K1, K2, V](
  ...
  override def accept(key1: K1, key2: K2, value: V, receiver: MultiOutputReceiver): Unit = {
    if (isPrimaryOnly) {
      ...
      receiver.get(tag.asInstanceOf[TupleTag[KV[KV[K1, K2], V]]]).output(KV.of(KV.of(key1, key2), value))
    }
  }
...
private[smb] class SMBFileOutputConsumer[K1, K2, V](
  ...
  sourceBucketMetadata: org.apache.beam.sdk.extensions.smb.BucketMetadata[_, _, _]  // For writing metadata.json
) extends SMBConsumer[K1, K2, V] {
  ...
  override def init(bucketIdParam: Int, effectiveParallelism: Int): Unit = {
    ...
    destination = SMBCollectionHelper.forBucket(fileAssignment, bucketShardId, effectiveParallelism, 1)
    ...
    val recordClass = specificData.getClass(outputMetadata.schema)
    ...
    val fileOps = if (outputMetadata.isParquet) {
      ...
      org.apache.beam.sdk.extensions.smb.AvroFileOperations.of(datumFactory, outputMetadata.schema)
        .asInstanceOf[org.apache.beam.sdk.extensions.smb.FileOperations[V]]
    }
  ...
  override def finish(): Unit = {
    ...
    if (bucketId == 0) {
      val metadataFile = fileAssignment.getDirectory.resolve("metadata.json", org.apache.beam.sdk.io.fs.ResolveOptions.StandardResolveOptions.RESOLVE_FILE)
  ...
  def getBucketFileInfo: (BucketShardId, ResourceId) = (bucketShardId, destination)
}
...
private[smb] case class SMBExecutionResult(
  ...
  writeResults: Map[SMBSaveAsSortedBucketOutput[_, _, _], org.apache.beam.sdk.extensions.smb.SortedBucketSink.WriteResult]
)
 ...
 * }}}
 *
 * The tap is created during graph construction (step 1) with knowledge of where
 * files will be written (via WriteResult metadata). The pipeline then executes
 * (step 2), and finally the tap reads the written files (step 3).
 */
...
private[smb] class SMBRootContext[K1, K2](
   ...
   */
  private[smb] def setCore(c: SMBCollectionCore[K1, K2, List[Iterable[Any]], List[Iterable[Any]]]): Unit = {
    require(core == null, "Core already set - this is a bug")
    core = c
  }

  /**
   * Seal the graph and execute it, returning the SMBExecutionResult with all outputs.
   * If already executed, returns the cached result.
   */
    ...
    if (!isSealed) {
      synchronized {
        if (!isSealed) {
          ...
          val result = SMBCollectionImpl.executeGraph(sources, targetParallelism, core, allSideInputs, keyClassSecondary)(sc)
...
private[smb] class SMBDeferredSCollection[K1: Coder, K2: Coder, V: Coder](
  ...
  def get: SCollection[((K1, K2), V)] = {
    ...
    if (rootContext.keyClassSecondary == classOf[Void]) {
      ...
      val typedPc = tuple.get(outputNode.tag).asInstanceOf[org.apache.beam.sdk.values.PCollection[KV[K1, V]]]
      ...
      val typedPc = tuple.get(outputNode.tag).asInstanceOf[org.apache.beam.sdk.values.PCollection[KV[KV[K1, K2], V]]]
...
/** Core tree node - stores transformation, children, and shared root context. Thin wrappers delegate to this. */
private[smb] sealed class SMBCollectionCore[K1: Coder, K2: Coder, In, Out: Coder](
  ...
  lazy val values: SMBCollectionValues[K1, K2, Out] = new SMBCollectionValuesWrapper[K1, K2, Out](this)

  /** Create transformation node. */
  def transform[W: Coder](f: (SideInputContext[_], K1, K2, Out) => TraversableOnce[W]): SMBCollectionCore[K1, K2, Out, W] = {
  ...
  /**
   * Create a deferred SCollection that will be materialized on first access.
   * Multiple deferred SCollections can share the same execution.
   */
  def toDeferredSCollection(): Deferred[SCollection[((K1, K2), Out)]] = {
    ...
    new SMBDeferredSCollection[K1, K2, Out](rootContext, outputNode, key1Coder, key2Coder, valueCoder)
  }

  /**
   * Attach a saveAsSortedBucket output to this node.
   * Returns a Deferred[ClosedTap] that will provide access to the written data.
   *
   * Usage pattern:
   * {{{
   ...
   * }}}
   *
   * The tap is created during graph construction (.get call) with metadata about where
   * files will be written. The pipeline then executes (sc.run()), and the tap can be
   * materialized to read the written files (result.tap()).
   */
  def attachSaveAsSortedBucket(output: SortedBucketIO.TransformOutput[K1, K2, Out]): Deferred[com.spotify.scio.io.ClosedTap[Out]] = {
    // Create output node as a child
    ...
    val deferredTap = new DeferredTap[Out](rootContext, outputNode, coderWithDatumFactory, outputMetadata)
  ...
  /**
   * Collect all side inputs from this node and all descendants.
   * Returns a deduplicated sequence of all side inputs in the tree.
   */
  ...
  def buildConsumer(
    ...
    fileOutputConsumers: mutable.Buffer[(SMBSaveAsSortedBucketOutput[K1, K2, _], SMBFileOutputConsumer[K1, K2, _])]
...
/**
 * Output node for toSCollectionAndSeal.
 * Extends SMBCollectionCore so it can be a child in the tree.
...
private[smb] class SMBSaveAsSortedBucketOutput[K1: Coder, K2: Coder, V: Coder](
  ...
  override def buildConsumer(
    ...
    val sourceBucketMetadata = {
      ...
      val bucketedInput = freshSource.toBucketedInput(org.apache.beam.sdk.extensions.smb.SortedBucketSource.Keying.PRIMARY)
    ...
    val consumer = new SMBFileOutputConsumer[K1, K2, V](outputMetadata, fileAssignment, sourceBucketMetadata)
...
/**
 * Keyed view - implements SMBCollection[K, Void, V] trait.
 * All methods delegate to the core.
 */
private[smb] class SMBCollectionKeyed[K1: Coder, K2: Coder, V: Coder](
  ...
  override def flatMap[W: Coder](f: (K1, K2, V) => TraversableOnce[W]): SMBCollection[K1, K2, W] =
    core.transform[W] { (_: SideInputContext[_], k1: K1, k2: K2, v: V) =>
      f(k1, k2, v)
    }.keyed

  override def values: SMBCollectionValues[K1, K2, V] = core.values

  override def saveAsSortedBucket(output: SortedBucketIO.TransformOutput[K1, K2, V]): Deferred[com.spotify.scio.io.ClosedTap[V]] = {
    core.attachSaveAsSortedBucket(output)
  }

  override def toDeferredSCollection()(implicit sc: ScioContext, coder: Coder[V]): Deferred[SCollection[((K1, K2), V)]] = {
...
private[smb] class SMBCollectionValuesWrapper[K1: Coder, K2: Coder, V: Coder](
  ...
  override def flatMap[W: Coder](f: V => TraversableOnce[W]): SMBCollectionValues[K1, K2, W] =
    core.transform[W] { (_: SideInputContext[_], _: K1, _: K2, v: V) =>
      f(v)
    }.values
  ...
  override def toDeferredSCollection()(implicit sc: ScioContext, coder: Coder[V]): Deferred[SCollection[V]] = {
...
private[smb] class SMBCollectionWithSideInputsWrapper[K1: Coder, K2: Coder, V: Coder](
  ...
  override def flatMap[W: Coder](f: (SideInputContext[_], K1, K2, V) => TraversableOnce[W]): SMBCollectionWithSideInputs[K1, K2, W] = {
    val newCore = new SMBCollectionCore[K1, K2, V, W](Some(ClosureCleaner.clean(f)), core.rootContext, sides)(core.key1Coder, core.key2Coder, implicitly[Coder[W]])
...
private[smb] class SMBCollectionWithSideInputsValuesWrapper[K1: Coder, K2: Coder, V: Coder](
  ...
  override def flatMap[W: Coder](f: (SideInputContext[_], V) => TraversableOnce[W]): SMBCollectionWithSideInputsValues[K1, K2, W] = {
