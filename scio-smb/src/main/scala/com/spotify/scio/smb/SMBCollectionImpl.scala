...
import org.apache.beam.sdk.extensions.smb.{BucketMetadata, BucketShardId, SmbValidationHelper, SortedBucketIO, SortedBucketSource, TargetParallelism}
...
private[smb] object SMBCollectionImpl {
  ...
  private[smb] def extractSourceMetadata(read: SortedBucketIO.Read[_]): SourceMetadata = {
    ...
    val isParquet = read.isInstanceOf[org.apache.beam.sdk.extensions.smb.ParquetAvroSortedBucketIO.Read[_]]
    SourceMetadata(tupleTag, schema, inputDirectories, isParquet)
  }

  /** Extract serializable metadata from a TransformOutput */
  private[smb] def extractOutputMetadata(output: SortedBucketIO.TransformOutput[_, _, _]): OutputMetadata = {
    import org.apache.beam.sdk.extensions.smb.SMBCollectionHelper
    val fileOps = SMBCollectionHelper.getFileOperations(output)
    val avroCoder = fileOps.getCoder().asInstanceOf[org.apache.beam.sdk.extensions.avro.coders.AvroCoder[_]]
    ...
    val isParquet = output.isInstanceOf[org.apache.beam.sdk.extensions.smb.ParquetAvroSortedBucketIO.TransformOutput[_, _, _]]
  ...
  private[smb] def reconstructRead(meta: SourceMetadata): SortedBucketIO.Read[_] = {
    ...
    val recordClass = specificData.getClass(meta.schema).asInstanceOf[Class[org.apache.avro.specific.SpecificRecord]]
    val tupleTag = meta.tupleTag.asInstanceOf[org.apache.beam.sdk.values.TupleTag[org.apache.avro.specific.SpecificRecord]]
  ...
  def createRoot[K1: Coder, K2: Coder, V: Coder](
    ...
    val rootContext = new SMBRootContext[K1, K2](sources, targetParallelism, keyClass, keyClassSecondary)(sc)

    // Fail-fast coder to catch serialization bugs early
    implicit val inOutCoder: Coder[List[Iterable[Any]]] =
      Coder.beam(new FailCoder[List[Iterable[Any]]](
        "List[Iterable[Any]]",
  ...
  class SortedBucketMultiTransformDoFn[K1, K2](
    ...
    fileOutputConsumersWithTags: Seq[(TupleTag[KV[BucketShardId, ResourceId]], SMBFileOutputConsumer[K1, K2, _])]
  ) extends org.apache.beam.sdk.transforms.DoFn[
    org.apache.beam.sdk.extensions.smb.SortedBucketTransform.BucketItem,
    org.apache.beam.sdk.values.KV[(K1, K2), Void]
  ] {
    ...
    def processElement(
      ...
      while (true) {
        val keyGroup = reader.readNext()
        if (keyGroup == null) {
          // Finish all consumers before returning
        ...
        valuesBySource.foreach {
          case exhaustable: ExhaustableLazyIterable[_] => exhaustable.exhaust()
  ...
  def executeGraph[K1, K2](
    ...
    val fileOutputConsumers = mutable.Buffer.empty[(SMBSaveAsSortedBucketOutput[K1, K2, _], SMBFileOutputConsumer[K1, K2, _])]
    ...
    val rootConsumer: SMBConsumer[K1, K2, List[Iterable[Any]]] = new PassThroughConsumer[K1, K2, List[Iterable[Any]]](
      children = childConsumers
    )

    val bucketFileTags = fileOutputConsumers.map { case (fileOutput, _) =>
      (fileOutput, new TupleTag[KV[BucketShardId, ResourceId]](s"bucket-files-${System.identityHashCode(fileOutput)}"))
    }.toMap
    ...
    val writeResults: Map[SMBSaveAsSortedBucketOutput[_, _, _], org.apache.beam.sdk.extensions.smb.SortedBucketSink.WriteResult] =
      fileOutputConsumers.map { case (fileOutput, _) =>
        val bucketFilesTag = bucketFileTags(fileOutput)
        val bucketFiles = pCollectionTuple.get(bucketFilesTag)
  ...
  def createSMBTransform[K1, K2](
    ...
    val buckets = sc.pipeline.apply("SMB-Read-Buckets", org.apache.beam.sdk.io.Read.from(bucketSource))
    ...
    val (javaKeyFn: java.util.function.Function[SortedBucketIO.ComparableKeyBytes, _], keyComparator) = if (keyClassSecondary == classOf[Void]) {
      ...
      val fn: java.util.function.Function[SortedBucketIO.ComparableKeyBytes, _] = SortedBucketIO.ComparableKeyBytes.keyFnPrimary(primaryKeyCoder)
      ...
      val fn: java.util.function.Function[SortedBucketIO.ComparableKeyBytes, _] = SortedBucketIO.ComparableKeyBytes.keyFnPrimaryAndSecondary(
        primaryKeyCoder,
        secondaryKeyCoder
      )
    ...
    // Main tag for unused output - DoFn outputs KV[(K1, K2), Void] where (K1, K2) is a Scala tuple
    // This is the same for both primary-only and composite keys
    val mainTag = new TupleTag[KV[(K1, K2), Void]]("main-unused")
    ...
    val sideTagList = allSideTags.foldLeft(org.apache.beam.sdk.values.TupleTagList.empty) { (list, tag) =>
      list.and(tag)
    }

    val parDo = org.apache.beam.sdk.transforms.ParDo.of(doFn)
    ...
    // Set coder for main tag - mainTag is TupleTag[KV[(K1, K2), Void]] where (K1, K2) is Scala tuple
    val mainPc = result.get(mainTag)
    val scalaK1K2Coder = com.spotify.scio.coders.Coder.tuple2Coder(key1Coder, key2Coder)
    val beamScalaK1K2Coder = com.spotify.scio.coders.CoderMaterializer.beam(sc, scalaK1K2Coder)
    mainPc.setCoder(
    ...
    outputNodes.foreach { node =>
      // Get value coder - use FileOperations for save nodes to preserve AvroCoder DatumFactory
      val beamValueCoder = node match {
        case saveNode: SMBSaveAsSortedBucketOutput[_, _, _] =>
          val fileOps = SMBCollectionHelper.getFileOperations(saveNode.output)
          val coder = fileOps.getCoder().asInstanceOf[org.apache.beam.sdk.coders.Coder[Any]]
          coder
        case _ =>
          // For non-save nodes (like toSCollection), use CoderMaterializer
          val valueCoder = node.valueCoder.asInstanceOf[Coder[Any]]
          val coder = com.spotify.scio.coders.CoderMaterializer.beam(sc, valueCoder)
          coder
      }
      ...
      result.get(node.tag).asInstanceOf[org.apache.beam.sdk.values.PCollection[Any]]
  ...
  def applyFinalization[K, K2, V](
    ...
    // Skip finalization - files are written directly to final location
    // WriteResult is not actually used since taps are populated directly from metadata
    // Just return a dummy empty WriteResult to satisfy the interface
