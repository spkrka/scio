...
/**
 * Represents a key specification for SMB operations.
 * Always has both primary (K1) and secondary (K2) key types.
 * For primary-key-only operations, K2 is Void.
 *
 * @param primaryClass The primary key class
 * @param secondaryClass The secondary key class (Void for primary-key-only)
 * @tparam K1 The primary key type
 * @tparam K2 The secondary key type (Void for primary-key-only)
 */
 ...
 *
 * @tparam T The type of value being deferred
 */
trait Deferred[+T] {
 ...
 * }}}
 *
 * @tparam K1 Primary key type for bucketing
 * @tparam K2 Secondary key type (use Void for primary-key-only)
 * @tparam V Value type
 */
@experimental
trait SMBCollection[K1, K2, V] {
  ...
  def flatMap[W](f: (K1, K2, V) => TraversableOnce[W])(implicit coder: Coder[W]): SMBCollection[K1, K2, W]
  ...
  def flatMapValues[W](f: V => TraversableOnce[W])(implicit coder: Coder[W]): SMBCollection[K1, K2, W] =
    flatMap((_, _, v) => f(v))
  ...
  /**
   * Value-only view where transformations don't expose keys.
   * Reduces boilerplate: `.values.filter(u => u.isActive)` vs `.filter((_, _, u) => u.isActive)`.
   */
  def values: SMBCollectionValues[K1, K2, V]

  /**
   * Write this collection to SMB files.
   * Executes automatically when `sc.run()` is called.
   *
   * @return Deferred ClosedTap that allows accessing written data after pipeline execution.
   *         Call `.get` on the returned Deferred AFTER `sc.run()` completes to access the tap.
   */
   ...
   * }}}
   */
  def toDeferredSCollection()(implicit sc: ScioContext, coder: Coder[V]): Deferred[SCollection[((K1, K2), V)]]

  /** Convert to SCollection immediately. Equivalent to `toDeferredSCollection().get`. */
  def toSCollectionAndSeal()(implicit sc: ScioContext, coder: Coder[V]): SCollection[((K1, K2), V)] =
...
trait SMBCollectionValues[K1, K2, V] {
  def flatMap[W](f: V => TraversableOnce[W])(implicit coder: Coder[W]): SMBCollectionValues[K1, K2, W]
...
object SMBCollection {
  ...
  /**
   * Create a Coder for Beam KV from component coders.
   * Used internally for secondary key support.
   */
  ...
  def cogroup4[K1: Coder, K2: Coder, A: Coder, B: Coder, C: Coder, D: Coder](
    ...
    targetParallelism: TargetParallelism = TargetParallelism.auto()
  )(implicit sc: ScioContext): SMBCollection[K1, K2, (Iterable[A], Iterable[B], Iterable[C], Iterable[D])] = {
    implicit val valueCoder: Coder[(Iterable[A], Iterable[B], Iterable[C], Iterable[D])] =
      Coder.tuple4Coder(Coder.iterableCoder[A], Coder.iterableCoder[B], Coder.iterableCoder[C], Coder.iterableCoder[D])

    SMBCollectionImpl.validateKeyClasses(List(a, b, c, d), key.primaryClass, Some(key.secondaryClass))
