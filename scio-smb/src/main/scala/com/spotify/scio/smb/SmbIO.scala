...
import org.apache.beam.sdk.extensions.smb.{AvroSortedBucketIO, BucketShardId}
...
object SmbIO {
   ...
   * }}}
   *
   * @param path Logical path used in test
   * @param keyClass Key class for bucketing
   * @param keyField Field name for the key
   ...
   *
   * In test mode, logical paths (like "gs://users") are resolved to temp directories
   * containing actual SMB files. In production mode, paths are returned unchanged.
   *
   ...
   * `.input(SmbIO(...), data)` AND prepareTestInput has been called manually.
   * Automatic resolution from TestDataManager is not currently supported due to
   * the need for key class and field name information.
   *
   * @param path Logical path (production) or test path
   * @param sc Scio context (to check if in test mode)
   * @return Physical path to read from
   */
   ...
   *
   * @param path Logical path (e.g., "gs://users")
   * @param keyClass Key class for bucketing
   * @param keyField Field name for the key
   * @param data Test data to write
   * @tparam K Key type
   * @tparam T Record type
   * @return Physical path to the temp directory
   */
    ...
    (_: ScioContext) => {
      ...
      new Tap[T] {
        override def value: Iterator[T] = {
          ...
          val filePattern = s"$outputPath/${outputMetadata.filenamePrefix}*${outputMetadata.filenameSuffix}"
  ...
  private[scio] def tap[K1, K2, T: Coder](
    ...
    (sc: ScioContext) => {
      // Reconstruct FileOperations fresh to get AvroCoder with DatumFactory
      // Do NOT close over FileOperations - it loses DatumFactory when serialized
      val fileOperations = write.getFileOperations()

      // Create coder from fresh FileOperations to preserve DatumFactory
