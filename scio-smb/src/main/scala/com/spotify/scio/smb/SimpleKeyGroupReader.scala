 ...
 *
 * Merges sorted key groups from multiple sources using linear scan across sources.
 * Internally uses lazy iterators wrapped in ExhaustableLazyIterable for memory efficiency,
 * but materializes to Seq at API boundary to maintain backward compatibility.
 *
 * Returns List[Seq[Any]] where index i corresponds to source i, eliminating the need
 * for CoGbkResult and TupleTag lookups.
 *
 * @param sources List of bucketed inputs (typically 2-3 sources for joins)
 * @param keyFn Function to convert ComparableKeyBytes to user key type K
 * @param keyComparator Comparator for ordering keys
 * @param metricsPrefix Prefix for metrics (e.g., "SMBCollection")
 * @param bucketId Bucket ID being processed
 * @param effectiveParallelism Effective parallelism for this transform
...
private[smb] class SimpleKeyGroupReader[K](
  ...
  /**
   * Iterator over a single source's key groups.
   * Wraps the underlying KeyGroupIterator.
   */
  private class SourceIterator(
    ...
    private val iter: java.util.Iterator[KV[SortedBucketIO.ComparableKeyBytes, java.util.Iterator[_]]] =
      source.createIterator(bucketId, parallelism, options).asInstanceOf[java.util.Iterator[KV[SortedBucketIO.ComparableKeyBytes, java.util.Iterator[_]]]]
    ...
    /**
     * Return a lazy iterator over all values for the current key from this source.
     * The iterator auto-advances to the next key group when exhausted.
     *
     * IMPORTANT: The returned iterator MUST be fully consumed before calling
     * peekKey() or consumeCurrentKeyGroupAsIterator() again.
     */
    ...
    /**
     * Consume ALL values for the current key from this source (eagerly materialized).
     * Automatically advances to the next key group.
     *
     * Deprecated: Use consumeCurrentKeyGroupAsIterator() for better memory efficiency.
     */
    def consumeCurrentKeyGroup(): Seq[Any] = {
      val values = mutable.Buffer.empty[Any]
      val valueIter = head.getValue
      // Consume ALL values for this key from this source
      while (valueIter.hasNext) {
        values += valueIter.next()
      }

      // Advance to next key group
      head = if (iter.hasNext) iter.next() else null
      values.toSeq
    }
  }
