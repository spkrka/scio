 ...
 * to temp directories and resolving logical paths (like "gs://users") to these temp paths.
 *
...
private[scio] object SmbTestPaths {
   ...
   *
   * @param testId Test identifier (typically from SmbIO.testId)
   * @param path Physical path to temp directory containing SMB files
   */
   ...
   * @return Some(path) if registered, None otherwise
   */
