 ...
 *
 * These are thin wrappers around cogroup that flatten the iterables.
 * Import com.spotify.scio.smb.syntax.all._ to use these methods.
 */
trait SMBCollectionSyntax {

  /**
   * Inner join - cartesian product of matching key groups.
   * Returns (K, (L, R)) for each pair where key matches.
   *
  ...
  def smbJoin[K: Coder: scala.reflect.ClassTag, L: Coder, R: Coder](
    ...
    SMBCollection
      ...
      .flatMapValues { case (lIter, rIter) =>
        for (l <- lIter; r <- rIter) yield (l, r)
      }
  }

  /**
   * Left outer join - all records from lhs, with optional matches from rhs.
   * Returns (K, (L, Option[R])).
   *
   * If no match in rhs, returns (l, None) for each l.
   * If matches exist in rhs, returns (l, Some(r)) for each pair.
   */
  @experimental
    ...
    SMBCollection
      ...
      .flatMapValues { case (lIter, rIter) =>
        ...
        if (rSeq.isEmpty) {
          ...
          for (l <- lIter; r <- rSeq) yield (l, Some(r))
        }
      }
  }

  /**
   * Right outer join - all records from rhs, with optional matches from lhs.
   * Returns (K, (Option[L], R)).
   */
  @experimental
    ...
    SMBCollection
      ...
      .flatMapValues { case (lIter, rIter) =>
        ...
        if (lSeq.isEmpty) {
          ...
          for (l <- lSeq; r <- rIter) yield (Some(l), r)
        }
      }
  }

  /**
   * Full outer join - all records from both sides, with optional matches.
   * Returns (K, (Option[L], Option[R])).
   *
    ...
    SMBCollection
      ...
      .flatMapValues { case (lIter, rIter) =>
        ...
        (lSeq.isEmpty, rSeq.isEmpty) match {
          ...
          case (false, false) => for (l <- lSeq; r <- rSeq) yield (Some(l), Some(r))
        }
      }
  }
}
