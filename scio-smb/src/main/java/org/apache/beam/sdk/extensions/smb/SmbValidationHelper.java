...
/**
 * Helper class for SMB validation operations.
 * Provides access to package-private Beam SMB classes from Scala code.
 */
public class SmbValidationHelper {
   ...
   *
   * This validates:
   * - Hash type compatibility (all sources must use same hash type)
   * - Bucket count compatibility (must be powers of 2)
   * - Key class compatibility
   *
   ...
   */
  public static BucketMetadata<?, ?, ?> getFirstMetadata(List<SortedBucketSource.BucketedInput<?>> sources) {
    if (sources.isEmpty()) {
      return null;
    }
   ...
   */
  public static <K> boolean keyClassMatches(BucketMetadata<?, ?, ?> metadata, Class<K> expectedKeyClass) {
   ...
   *
   * <p>For backward compatibility: when expecting Void.class (primary-only keys),
   * we accept sources with either Void.class or null secondary key class.
   *
   * @param metadata the bucket metadata to check
   * @param expectedSecondaryKeyClass the expected secondary key class
   * @return true if secondary key classes match
   */
  public static <K> boolean keyClassSecondaryMatches(BucketMetadata<?, ?, ?> metadata, Class<K> expectedSecondaryKeyClass) {
