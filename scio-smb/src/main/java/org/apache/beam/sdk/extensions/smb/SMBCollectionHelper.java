...
/**
 * Helper class to expose package-private SMB methods to Scala SMBCollection implementation.
 *
 * This class provides access to package-private methods of TransformOutput and other
 * SMB classes that need to be accessed from com.spotify.scio.smb package.
 */
public class SMBCollectionHelper {
   ...
   */
  public static SourceSpec createSourceSpec(java.util.List<SortedBucketSource.BucketedInput<?>> inputs) {
   ...
   */
  public static org.apache.beam.sdk.coders.Coder<?> getPrimaryKeyCoder(BucketMetadata<?, ?, ?> metadata) {
   ...
   */
  public static org.apache.beam.sdk.coders.Coder<?> getSecondaryKeyCoder(BucketMetadata<?, ?, ?> metadata) {
    return metadata.getKeyCoderSecondary();
  }

  /**
   * Extract primary key coder from SMB metadata using keyClassMatches.
   * This searches all BucketedInput sources for a metadata with matching primary key class.
   */
  public static <K1> Coder<K1> getPrimaryKeyCoder(
      List<SortedBucketSource.BucketedInput<?>> inputs,
      Class<K1> keyClass
  ) {
    Optional<Coder<K1>> coder = inputs.stream()
    ...
    return coder.orElseThrow(() -> new IllegalStateException(
        "Could not infer key coder for class " + keyClass + " from SMB metadata"
    ));
  }

  /**
   * Extract secondary key coder from SMB metadata using keyClassSecondaryMatches.
   * This searches all BucketedInput sources for a metadata with matching secondary key class.
   */
  public static <K2> Coder<K2> getSecondaryKeyCoder(
      List<SortedBucketSource.BucketedInput<?>> inputs,
      Class<K2> keyClassSecondary
  ) {
    Optional<Coder<K2>> coder = inputs.stream()
        .flatMap(i -> i.getSourceMetadata().mapping.values().stream())
        .filter(sm -> sm.metadata.getKeyClassSecondary() != null
    ...
    return coder.orElseThrow(() -> new IllegalStateException(
        "Could not infer secondary key coder for class " + keyClassSecondary + " from SMB metadata"
    ));
  }
   ...
   */
  public static <V> org.apache.beam.sdk.transforms.PTransform<
      org.apache.beam.sdk.values.PCollection<org.apache.beam.sdk.values.KV<BucketShardId, org.apache.beam.sdk.io.fs.ResourceId>>,
   ...
   */
  public static org.apache.beam.sdk.coders.Coder<org.apache.beam.sdk.values.KV<BucketShardId, org.apache.beam.sdk.io.fs.ResourceId>> getBucketFilesCoder() {
  ...
  /**
   * Get Schema from an AvroSortedBucketIO.Read or ParquetAvroSortedBucketIO.Read (used for source metadata extraction).
   */
  public static org.apache.avro.Schema getSchema(SortedBucketIO.Read<?> read) {
    ...
    if (read instanceof ParquetAvroSortedBucketIO.Read) {
      ...
      if (recordClass != null) {
        try {
          return ((org.apache.avro.specific.SpecificRecord) recordClass.getDeclaredConstructor().newInstance()).getSchema();
        } catch (Exception e) {
          throw new RuntimeException("Failed to derive schema from record class " + recordClass, e);
        }
      }
      throw new IllegalArgumentException("ParquetAvroSortedBucketIO.Read must have either schema or recordClass");
    }
    throw new IllegalArgumentException("Only AvroSortedBucketIO.Read and ParquetAvroSortedBucketIO.Read are currently supported");
  }

  /**
   * Get input directories from an AvroSortedBucketIO.Read or ParquetAvroSortedBucketIO.Read (used for source metadata extraction).
   */
